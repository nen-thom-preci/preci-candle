            ...
            <nav className="flex items-center gap-6">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="text-base font-brand font-bold text-foreground hover:text-primary transition-colors"
                >
                  {item.label}
                </Link>
              ))}
          ...
          <nav className="md:hidden border-t border-border mt-4 pt-4 space-y-1 bg-background animate-in slide-in-from-top-2">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="block text-base font-bold text-foreground hover:text-primary hover:bg-secondary/50 px-3 py-2 rounded-md transition-colors"
